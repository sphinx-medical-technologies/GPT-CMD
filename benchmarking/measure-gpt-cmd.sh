...
while [ $i -lt $count ];do
	...
	echo "prompt [$prompt]"
	response_time=$(curl -X "POST" -w "$i: %{time_total} %{http_code} %{size_download} %{url_effective}\n" -H "accept: application/json"  -H "Content-Type: application/json" -d "{ \"prompt\": [ \"$prompt\" ], \"bad_words\": [], \"temperature\": 0.8, \"top_k\": 50, \"top_p\": 1, \"min_length\": 0, \"max_length\": 20, \"penalty_alpha\": 0, \"repetition_penalty\": 1, \"early_stop\": false, \"end_sequence\": \"\", \"do_sample\": true, \"num_beams\": 1, \"return_prompt\": \"\", \"seed\": -1 }" -o gen-measure.json -s "http://0.0.0.0:9002/generate/")
